		...
		$("#applyExpanders")
				.click(
						function() {
							var actions = "";
							$(
									"#expanderOptions input.selected-expander-option:not(:checked)")
									.each(function() {
										actions += "&" + $(this).val();
									});
							$(
									"#expanderOptions input.unselected-expander-option:checked")
									.each(function() {
										actions += "&" + $(this).val();
									});
							var currentPageState = $("#currentState").val();
		...
		$('.actionSelect').change(
				function() {
					...
					window.location = [ location.protocol, "//", location.host,
							location.pathname, "?", currentPageState,
							viewAction ].join('');
					return false;
				});
					...
					<div class="table-row">
						<div class="table-cell">
							...
							<div class="statistics">
								<strong> <%
 	out.println(resultList.getHits());
 %>Results
									for.... <br /> <%=value%>
								</strong>
							</div>
 	...
 	out.println("Limiter: " + limiterName);
 %></a>
										</p></li>
									<%
										}
 ...
 %></a>
						...
						<div class="table-cell">
							<div class="top-menu">
								...
								<!------------------------------------------ Research Starter --------------------------------->
								<%
									if(resultList.getRecordsList().size() < researchStarters)
									{
								...
								<div data-placard="<%query.toString();%>"
									...
									<section class="placard">
										<h2 class="hidden">Placard</h2>
										<%
											if(resultList.getRecordsList().get(i).getImageInfo()!=null){
										%>
											...
											<div class="links">
												...
												<%
													if(!result.getCustomLinkList().isEmpty()){
												                                                    for(CustomLink link : result.getCustomLinkList()){
												%>
												<img src= "<% out.println(link.getIcon()); %>" />
												<a class="customLink" title = "<%out.println(link.getMouseOverText()); %>"
													href="<%out.println(link.getUrl()); %>"><%=link.getText()%>
													</a>
												<%
													}
												                                                    }
